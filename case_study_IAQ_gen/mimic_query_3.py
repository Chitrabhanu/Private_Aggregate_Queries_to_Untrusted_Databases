...
filename = 'max_of_urgent_admissions'
...
for i in range(1):
    ...
    for ad_type in unique_ad_type:
        most_recent_time[ad_type] = max(database[database[filter_field] == ad_type]["ADMITTIME"])
    for i in range(len(filter_field_list)):
        # if database["ADMITTIME"].loc[i] == most_recent_time[filter_field_list[i]]:

...
struct _SpMV_specializer<T, 0>
{
                                       ...
                                       const uint *rows)
    {
        ...
        if (DEBUG_IF_ENABLED && i == DEBUG_IDX)
        {
            printf("\n  a_lo:  \t");
            _print_limbs<T>(res_lo.lo, LIMBS_PER_UINTX);
        }
        if (DEBUG_IF_ENABLED && i == DEBUG_IDX)
        {
            printf("\n  a_hi:  \t");
            _print_limbs<T>(res_hi, LIMBS_PER_UINTX);
        }
        if (DEBUG_IF_ENABLED && i == DEBUG_IDX)
        {
            printf("\n  over:  \t%u", overflow);
        ...
        {
            printf("\n  sub_lo:\t");
            _print_limbs<T>(subtrahends[overflow], LIMBS_PER_UINTX);
        }
        if (DEBUG_IF_ENABLED && i == DEBUG_IDX)
        {
            printf("\n  sub_hi:\t");
            _print_limbs<T>(subtrahends[2 * overflow + 1], LIMBS_PER_UINTX);
        }
        ...
        {
            printf("\n  q_lo:  \t");
            _print_limbs<T>(q.lo, LIMBS_PER_UINTX);
        }
        if (DEBUG_IF_ENABLED && i == DEBUG_IDX)
        {
            printf("\n  q_hi:  \t");
            _print_limbs<uint>(q.hi, 1);
        }
        uintXp<T> r2 = get_r2(q, *modulus);
        if (DEBUG_IF_ENABLED && i == DEBUG_IDX)
        {
            printf("\n  r2:    \t");
            _print_limbs<uintXp<T>>(r2, LIMBS_PER_UINTX + 1);
        }
        ...
        {
            printf("\nCUDA Kernel  a''': \t");
            _print_limbs<T>(response[i], LIMBS_PER_UINTX);
        }
    }
};
...
template <>
struct _SpMV_specializer<GF28_Element, 0>
{
                                       ...
                                       const uint ncols, const uint *cols, const uint *rows)
    {
        ...
        response[i] = res;
        if (!DEBUG_IF_ENABLED && i == DEBUG_IDX)
            printf("device_SpMV:GF28_Element:GPU response[%d]: %u\n", i, response[i]);
    }
};

// specialization for GF216_Element
template <>
struct _SpMV_specializer<GF216_Element, 0>
{
...
// specialization for uintX
template <typename T>
struct _truncateBits_specializer<T, 0>
{
...
// specialization for GF28_Element
template <>
struct _truncateBits_specializer<GF28_Element, 0>
{
...
// specialization for GF216_Element
template <>
struct _truncateBits_specializer<GF216_Element, 0>
{
...
// specialization for uintX
template <typename T>
struct _maxOverflow_specializer<T, 0>
{
...
// specialization for GF28_Element
template <>
struct _maxOverflow_specializer<GF28_Element, 0>
{
...
// specialization for GF216_Element
template <>
struct _maxOverflow_specializer<GF216_Element, 0>
{
              ...
              const SparseMatrix<T> &matrix)
{
    for (int i = 0; i < matrix.ncols; i++)
    {
        ...
        {
            std::cout << "\nNTL  a''': \t";
            print_limbs<T>(response[i], LIMBS_PER_UINTX);
        }
    }
}
                      ...
                      const SparseMatrix<T> &matrix, struct BarrettParams<T> &barrett)
{
    ...
    for (int i = 0; i < matrix.ncols; i++)
    {
        ...
        {
            std::cout << "\n  a_lo:  \t";
            print_limbs<T>(response_ZZ[i], LIMBS_PER_UINTX);
        }
        if (DEBUG_IF_ENABLED && i == DEBUG_IDX)
        {
            std::cout << "\n  a_hi:  \t";
            print_limbs<T>(response_ZZ[i] >> BITS_IN(LIMBS_PER_UINTX), LIMBS_PER_UINTX);
        }
        ...
        {
            std::cout << "\n  sub_lo:\t";
            print_limbs<T>(barrett.l_subtrahends[overflow], LIMBS_PER_UINTX);
        }
        if (DEBUG_IF_ENABLED && i == DEBUG_IDX)
        {
            std::cout << "\n  sub_hi:\t";
            print_limbs<T>(barrett.l_subtrahends[2 * overflow + 1] >> BITS_IN(LIMBS_PER_UINTX), LIMBS_PER_UINTX);
        }
        ...
        {
            std::cout << "\n  q_lo:  \t";
            print_limbs<T>(q3, LIMBS_PER_UINTX);
        }
        if (DEBUG_IF_ENABLED && i == DEBUG_IDX)
        {
            std::cout << "\n  q_hi:  \t";
            print_limbs<uint>(q3 >> BITS_IN(LIMBS_PER_UINTX), 1);
        }
        ...
        {
            std::cout << "\n  r2:    \t";
            print_limbs<uintXp<T>>(r2, LIMBS_PER_UINTX + 1);
        }
        NTL::ZZ r = (r1 - r2) % NTL::power2_ZZ(BITS_IN(LIMBS_PER_UINTX + 1));
        if (DEBUG_IF_ENABLED && i == DEBUG_IDX)
        {
            std::cout << "\n  a_lo'':\t";
            print_limbs<T>(r, LIMBS_PER_UINTX);
        }
        if (DEBUG_IF_ENABLED && i == DEBUG_IDX)
        {
            std::cout << "\n  a_hi'':\t";
            print_limbs<uint>(r >> BITS_IN(LIMBS_PER_UINTX), 1);
        }
...
#ifdef GF28
    struct SparseMatrix<GF28_Element> matrix =
    {
        0
    };
    NTL::ZZ modulus;
...
#ifdef GF216
    struct SparseMatrix<GF216_Element> matrix =
    {
        0
    };
    NTL::ZZ modulus;
...
/*
    //written in by Warren:
    for (int i = 0; i < nstreams * matrix.nrows; i++)
    { 
#ifdef uintX
        to_uint<uintX>(1, l_query[i]);
        std::cout<<"uintX"<<"\n";
#endif
#ifdef GF28
        l_query[i] = 0;
...
*/
    ...
    std::cout<<"Matrix l_rows: ";
    for(int i = 0; i < 12; i++) {std::cout<< matrix.l_rows[i] <<" ";}
    std::cout<<"\n";
    
    std::cout << "l_rows: ";
    for(int i = 0;i< 12;i++){
        print_limbs<uint>(matrix.l_rows[i], LIMBS_PER_UINTX);
    }
    std::cout<<"\n";

    
    std::cout<<"Matrix l_cols: ";
    for(int i = 0; i < 8; i++) {std::cout<< matrix.l_cols[i]<<" ";}
    std::cout<<"\n";
    
    std::cout << "l_cols: ";
    for(int i = 0;i< 8;i++){
        print_limbs<uint>(matrix.l_cols[i], LIMBS_PER_UINTX);
    }
    std::cout<<"\n";
    std::cout<<"Matrix l_vals: ";
    for(int i = 0; i < 12; i++) {std::cout<< matrix.l_vals[i]<<" ";}
    std::cout<<"\n";
    
    std::cout << "l_vals: ";
    for(int i = 0;i< 12;i++){
        print_limbs<uint>(matrix.l_vals[i], LIMBS_PER_UINTX);
    }
    std::cout<<"\n";

    std::cout << "l_query: ";
    for(int i = 0;i< matrix.nrows;i++){
        print_limbs<uint>(l_query[i], LIMBS_PER_UINTX);
    }
    std::cout<<"\n";
...
#pragma omp parallel
    ...
    std::cout << "\nl_response: ";
    for(int i = 0; i<sizeof(l_response);i++){std::cout<<l_response[i]<<" ";}
    std::cout <<"\n";
    //std::cout <<*l_response;
    std::cout << "l_response: ";
    for(int i = 0;i< sizeof(l_response);i++){
        print_limbs<uint>(l_response[i], LIMBS_PER_UINTX);
    }
    std::cout<<"\n";
                ...
                struct SparseMatrix<T> &matrix, uint &max_overflow)
{
    ...
    if (!valstream)
    {
        cerr << "Error: CCS representation: can't open VALS file\n";
        exit(-1);
    }
    std::ifstream rowstream(rowfile, std::ifstream::in);
    if (!rowstream)
    {
        cerr << "Error: CCS representation: can't open ROWS file\n";
        exit(-1);
    }
    std::ifstream colstream(colfile, std::ifstream::in);
    if (!colstream)
    {
        cerr << "Error: CCS representation: can't open COLS file\n";
        exit(-1);
    }
                 ...
                 const uint max_overflow)
{
    ...
    for (int i = 0; i <= max_overflow; ++i)
    {
        barrett.l_subtrahends[i] = ((NTL::to_ZZ(i)
                                     << (2 * BITS_PER_LIMB * LIMBS_PER_UINTX)) /
                                    modulus_zz) *
                                   modulus_zz;
